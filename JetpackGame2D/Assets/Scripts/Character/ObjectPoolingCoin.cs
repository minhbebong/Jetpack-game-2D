...
public class ObjectPoolingCoin : MonoBehaviour
{
    public List<GameObject> coinPrefabs; // List of different coin prefabs
    public int poolSizePerCoin = 10; // Number of coins to pool per coin prefab
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < coinPrefabs.Count; i++)
        {
            ...
            for (int j = 0; j < poolSizePerCoin; j++)
            {
                GameObject coin = Instantiate(coinPrefabs[i]);
